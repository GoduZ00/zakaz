    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 mb-8">
      <div className="flex justify-center gap-4 lg:gap-8 flex-wrap">
        {categories.map((cat, idx) => (
          <Link
            key={idx}
            to={`/catalog/${idx === 0 ? 'mekhanicheskie_torgovye_avtomaty_catalog' : 'napolniteli-dlya-torgovykh-avtomatov'}`}
            className="flex flex-col items-center group bg-white border border-gray-100 p-6 rounded hover:shadow-lg transition-shadow w-full max-w-[250px]"
          >
